"use strict";
(function () {
    angular.module("burnIt.trackfood").controller("NutritionFactCtrl", ["CommonSvc", "$location", "$http", NutritionFactCtrl]);
   
    function NutritionFactCtrl(CommonSvc, $location, $http) {
        var vm = this;
        vm.route = route;
    ...
    (function () {
        for (var x = 0; x < 5; x++) {
            if (x < 2) {
                obj = JSON.stringify(CommonSvc.getFoodtrack());
                console.log(obj);
            }
            else {
                if (obj == null) {}
